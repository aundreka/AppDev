...
const String CUPCAKE_SPRITE_PATH =
    'activity2/monsters/level2/cupcake.png';
const String CAKE_SPRITE_PATH = 'activity2/monsters/level2/cake.png';
const String ICECREAM_SPRITE_PATH =
...
class Level2Map extends FlameGame {
  ...
  int _currentScore = 0;
  int _highScore = 0;
  static const _prefsKeyHighScore = 'level2_high_score';
  ...
  Future<void> onLoad() async {
    ...
    FlameAudio.bgm.stop();
    FlameAudio.bgm.play('level2_theme.mp3');
  }
  ...
  void _runSpawns() {
    ...
    while (_t >= _nextPreW2 && _t < 90) {
      _nextPreW2 += 1;
      _spawn(EnemyKind.cupcake, spawnNear());
      _spawn(EnemyKind.cake, spawnNear());
  ...
  void _spawn(EnemyKind kind, Vector2 pos) {
    ...
    if (kind == EnemyKind.boss) {
      FlameAudio.play('level2_boss.mp3');
    }
  }
...
const Map<EnemyKind, EnemyStats> kEnemyStats = {
  ...
  EnemyKind.cake: EnemyStats(350, 40, 2, 75, 80),
  EnemyKind.icecream: EnemyStats(700, 60, 2.2, 70, 140),
  EnemyKind.boss: EnemyStats(12000, 300, 3, 80, 0),
};

class Enemy extends PositionComponent with HasGameRef<Level2Map> {
  ...
  Enemy(this.kind, Vector2 pos)
      : super(
          position: pos,
          size: Vector2.all(kind == EnemyKind.boss ? 210 : 110),
